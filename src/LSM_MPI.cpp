...
int main(int argc, char **argv)
{
    ...
    std::cout << "SIZE: " << size + stride << std::endl;
    ...
    double variancePilot = arma::var(LSMprice) / numSimsPilot;
...
//        }
    ...
    LSM_mlmc* myLSM_mlmc = new LSM_mlmc(l, kappa);
    ...
    if (mpiroot) {
        ...
        LSMprice = myLSM->computeLSM(myOption, stock, beta);
        priceMLMC = arma::mean(LSMprice);
        variance = arma::var(LSMprice) / numSims;
    }
    else {
        ...
        correlation = myLSM_mlmc->correlation;
    }
    MPI_Barrier(MPI_COMM_WORLD);
