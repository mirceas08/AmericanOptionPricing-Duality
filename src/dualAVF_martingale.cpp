...
#include "mlmc/mlmc.h"
#include "mlmc/dualAVF_martingale.h"
...
int main(int argc, char **argv)
{
    ...
    numSuccessors = std::ceil(numSuccessors / std::pow(kappa, size - (rank+1)));
    ...
    if (mpiroot) {
        ...
        double nominatorN0 = std::pow(10, 6);
...
//    }
    ...
    /* ***************** MLMC estimator ******************** */
    double priceMLMC;
    DualAVF_martingale* myDualAVF_martingale = new DualAVF_martingale(l, kappa, numSuccessors);
    ...
    if (mpiroot) {
        ...
        arma::vec mlmcPrice = myDualAVF_martingale->computeDualAVF_martingale(myOption, stock, beta);
        priceMLMC = arma::mean(mlmcPrice);
        variance = arma::var(mlmcPrice) / numSims;
        correlation = myDualAVF_martingale->correlation;
    }
    MPI_Barrier(MPI_COMM_WORLD);
    ...
    if (mpiroot) {
        ...
        std::cout << "Level" << setw(12) << "# paths" << setw(14) << "Successors" << setw(12) << "Price" << setw(23) << "Variance"
        ...
        for (int i = 0; i < size; i++) {
            ...
            printElement(numSuccessors * std::pow(kappa, i), 14);
