...
int main(int argc, char **argv)
{
    ...
    if (mpiroot) {
        ...
        std::cout << "Number of successors in Dual SR: " << subSims << std::endl;
...
//        }
    ...
    DualSR_mlmc* myDualSR_mlmc = new DualSR_mlmc(l, kappa, subSims);
    ...
    if (mpiroot) {
        ...
        arma::vec mlmcPrice = myDualSR_mlmc->computeDualSR_mlmc(myOption, S, betaML);
        priceMLMC = arma::mean(mlmcPrice);
        variance = arma::var(mlmcPrice) / numSims;
        correlation = myDualSR_mlmc->correlation;
    }
    MPI_Barrier(MPI_COMM_WORLD);
